                                    ...
                                    <label for="exampleInputPassword1">Thuộc danh mục</label>
                                      <select name="category_parent" class="form-control input-sm m-bot15">
                                        ...
                                        @foreach($category as $key => $val)
                                            ...
                                            <option {{$val->category_parent==$edit_value->category_id ? 'selected' : '' }} value="{{$val->category_id}}">{{$val->category_name}}</option>
                                            @endif

                                            @foreach($category as $key => $val2)
                                                @if($val2->category_parent==$val->category_id)
                                                    <option {{$val2->category_parent==$edit_value->category_id ? 'selected' : '' }} value="{{$val2->category_id}}">-----{{$val2->category_name}}-----</option>
                                                @endif
                                            @endforeach
                                        @endforeach   
                                    </select>
